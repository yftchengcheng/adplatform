        ...
        <div className="flex items-center gap-1 p-0.5 bg-gray-100 rounded-lg">
            <button
                ...
                className={cn(
                    ...
                    value === "input" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"
                )}>z手动输入</button>
            <button
                onClick={() => onChange("macro")}
                className={cn(
                    "px-3 py-1 text-xs font-medium rounded-md transition-all",
                    value === "macro" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"
                )}>宏替换
                      </button>
        </div>
    );
...
export function TreasureboxRainTemplateConfigPanel(
    ...
    const [guideTextMode, setGuideTextMode] = useState<"input" | "macro">(config.guideTextMacro ? "macro" : "input");
    const [rewardTextMode, setRewardTextMode] = useState<"input" | "macro">(config.rewardTextMacro ? "macro" : "input");
    const [specialNoteMode, setSpecialNoteMode] = useState<"input" | "macro">(config.specialNoteMacro ? "macro" : "input");
    const [cashMode, setCashMode] = useState<"input" | "macro">(config.cashAmountMacro ? "macro" : "input");
    const [landingMode, setLandingMode] = useState<"input" | "macro">(config.landingPageMacro ? "macro" : "input");
    const [rewardImageMode, setRewardImageMode] = useState<"input" | "macro">(config.rewardImageMacro ? "macro" : "input");
    const [treasureboxImageMode, setTreasureboxImageMode] = useState<"input" | "macro">(config.treasureboxImageMacro ? "macro" : "input");

    const updateConfig = useCallback((updates: Partial<TreasureboxRainConfig>) => {
        onChange({
            ...config,
            ...updates
        });
        ...
        <div className="space-y-4">
            ...
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                <div className="px-4 py-2 bg-gray-50 border-b border-gray-200">
                    <SectionHeader title="基础配置" isOpen={basicOpen} onToggle={() => setBasicOpen(!basicOpen)} />
                </div>
                {basicOpen && <div className="p-4 space-y-4">
                    {}
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-700">宝箱图片</label>
                        <div className="flex items-center gap-2 mb-2">
                            <ModeToggle value={treasureboxImageMode} onChange={setTreasureboxImageMode} />
                        </div>
                        {treasureboxImageMode === "input" ? <div className="space-y-2">
                            <div
                                className="border-2 border-dashed border-gray-200 rounded-lg overflow-hidden">
                                {config.treasureboxImageUrl ? <div className="relative group p-2">
                                    <img
                                    ...
                                    <button
                                        ...
                                        className="absolute top-2 right-2 w-6 h-6 bg-red-500 text-white rounded-full text-xs opacity-0 group-hover:opacity-100 transition-opacity">X
                                                                </button>
                                </div> : <label
                                    ...
                                    <input
                                        ...
                                        onChange={e => {
                                            ...
                                            if (file) {
                                                ...
                                                reader.onload = ev => {
                                                    updateConfig({
                                                        treasureboxImageUrl: ev.target?.result as string
                                                    });
                                                };
                            ...
                            <Input
                                ...
                                onChange={e => updateConfig({
                                    treasureboxImageMacro: e.target.value
                                })}
                    ...
                    <div className="space-y-2">
                        ...
                        {guideTextMode === "input" ? <div className="space-y-1">
                            <Input
            ...
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                ...
                {rewardOpen && <div className="p-4 space-y-4">
                    {}
                    <div className="space-y-2">
                    ...
                    {}
                    {config.rewardType === "cash" && <div className="space-y-2">
                        <div className="flex items-center justify-between">
                            <label className="text-sm font-medium text-gray-700">现金金额</label>
                            <ModeToggle value={cashMode} onChange={setCashMode} />
                        </div>
                        {cashMode === "input" ? <div className="space-y-1">
                            ...
                            <Input
                                ...
                                onChange={e => updateConfig({
                                    cashAmountMacro: e.target.value
                                })}
                    ...
                    {}
                    {config.rewardType === "custom" && <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-700">奖励图片</label>
                        <div className="flex items-center gap-2 mb-2">
                            <ModeToggle value={rewardImageMode} onChange={setRewardImageMode} />
                        </div>
                        {rewardImageMode === "input" ? <div className="space-y-2">
                            <div
                                className="border-2 border-dashed border-gray-200 rounded-lg overflow-hidden">
                                {config.rewardImageUrl ? <div className="relative group p-2">
                                    <img
                                    ...
                                    <input
                                        ...
                                        onChange={e => {
                                            ...
                                            if (file) {
                                                ...
                                                reader.onload = ev => {
                                                    updateConfig({
                                                        rewardImageUrl: ev.target?.result as string
                                                    });
                                                };
                            ...
                            <Input
                                ...
                                onChange={e => updateConfig({
                                    rewardImageMacro: e.target.value
                                })}
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center justify-between">
                            ...
                            <ModeToggle value={rewardTextMode} onChange={setRewardTextMode} />
                        </div>
                        {rewardTextMode === "input" ? <div className="space-y-1">
                            <Input
                            ...
                            <Input
                                ...
                                onChange={e => updateConfig({
                                    rewardTextMacro: e.target.value
                                })}
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center justify-between">
                            ...
                            <ModeToggle value={specialNoteMode} onChange={setSpecialNoteMode} />
                        </div>
                        {specialNoteMode === "input" ? <div className="space-y-1">
                            <Input
                            ...
                            <Input
                                ...
                                onChange={e => updateConfig({
                                    specialNoteMacro: e.target.value
                                })}
            ...
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                ...
                {landingOpen && <div className="p-4 space-y-4">
                    <div className="flex items-center gap-2 mb-2">
                        <ModeToggle value={landingMode} onChange={setLandingMode} />
                    </div>
                    {landingMode === "input" ? <div className="space-y-2">
                        <div className="relative">
                            <Link2
                        ...
                        <Input
                            ...
                            onChange={e => updateConfig({
                                landingPageMacro: e.target.value
                            })}
            ...
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                <div className="px-4 py-2 bg-gray-50 border-b border-gray-200">
                    <SectionHeader title="其他配置" isOpen={otherOpen} onToggle={() => setOtherOpen(!otherOpen)} />
                </div>
                {otherOpen && <div className="p-4 space-y-4">
                    {}
                    <div className="space-y-2">
            ...
            {}
            {(onSave || onCancel) && <div className="flex gap-3 pt-4 mt-4 border-t border-gray-200">
                {onCancel && <button
                    onClick={() => setShowCancelConfirm(true)}
                    className="flex-1 h-10 px-4 rounded-lg border border-gray-300 text-gray-600 text-sm font-medium hover:bg-gray-50 transition-colors">取消
                                </button>}
                {onSave && <button
                    onClick={onSave}
                    className="flex-1 h-10 px-4 rounded-lg bg-blue-500 text-white text-sm font-medium hover:bg-blue-600 transition-colors">保存
                                </button>}
            </div>}
            {}
            {showCancelConfirm && <>
                    ...
                    <div className="relative bg-white rounded-2xl shadow-2xl overflow-hidden">
                        <div className="px-5 pt-6 pb-4">
                            ...
                            <div className="flex gap-3">
                                <button
                                    onClick={() => setShowCancelConfirm(false)}
                                    className="flex-1 h-10 px-4 rounded-lg border border-gray-300 text-gray-600 text-sm font-medium hover:bg-gray-50 transition-colors">继续编辑
                                                      </button>
                                <button
                                    onClick={() => {
                                    ...
                                    className="flex-1 h-10 px-4 rounded-lg bg-red-500 text-white text-sm font-medium hover:bg-red-600 transition-colors">确认取消
                                                      </button>
                            </div>
                        </div>
                    </div>
