...
## 二十二、下载六要素 v6 贴边 Smart App Banner（2025-01 迭代）

### 1. 核心约束（用户已重复 2 次强调）
- **预览不能占满屏，会遮盖广告主体**
- **必须贴边（顶部或底部）的窄条**
- **LOGO 非必填，有/无 LOGO 必须视觉对齐**
- 最终落地：**单行主条 66px**，6 要素收纳到 ⓘ 折叠面板

### 2. 高度演进
| 版本 | 高度 | 形态 | 反馈 |
|------|------|------|------|
| v3 | 500+ px | 全屏卡片 | ❌ 占满屏 |
| v4 | 388 px | 4 段堆叠 | ❌ 还是偏大 |
| v5 | 143 px | 3 段 banner | ❌ 用户说还是偏大 |
| **v6** | **66 px** | **贴边 Smart App Banner + ⓘ 折叠** | ✅ 满足 |

### 3. v6 视觉结构
```
┌──────────────────────────────────────────────────────────┐
│ [LOGO] 应用名称 [4+]   开发者 · v1.0.0   [ⓘ] [立即下载] │  ← 主条 66px
└──────────────────────────────────────────────────────────┘
   ↑                                          ↑
   有/无 LOGO 高度差 = 0px                  ⓘ 点击展开 ↓
                                            ┌──────────────────────┐
                                            │ ✨ 功能1 · 功能2 ...  │
                                            │ 🛡️ 隐私 | 🔑 权限 | 🏛 备案 │
                                            └──────────────────────┘
...
### 4. 主条 4 段布局
- **LOGO**：`w-11 h-11` 容器固定，有图渲染图片，无图显示 Package 图标（背景 `${primaryColor}1A`）
- **名称 + 副标**：flex-1，13px 名称 + 10px 副标，2 行垂直
- **ⓘ 详情按钮**：`w-6 h-6` 圆按钮，`text-gray-400 hover:bg-gray-100`
- **下载按钮**：`h-7 px-3`，渐变 `${primaryColor} → ${primaryColor}dd`，按压 `scale(0.96)`

### 5. 折叠面板（点击 ⓘ 展开）
- 触发：单个 `useState` toggle，点击关闭再次点击
- 高度：默认 0，展开时 100-150px
- 内容：产品功能 chip + 隐私/权限/备案 链接条
- z-index：`z-30` 在最上层

### 6. 数据属性（Playwright 验证用）
- `data-d6e-root`：主容器
- `data-d6e-top-row`：主条
- `data-d6e-toggle`：ⓘ 按钮
- `data-d6e-detail`：折叠面板

### 7. 性能/UX 细节
- LOGO 容器固定 44x44，**有/无 LOGO 高度差 0px**（避免抖动）
- 主条 padding `p-2.5`（10px），内部 gap `gap-2.5`
- 折叠面板 `bg-gray-50/60` 浅灰底，与白底主条区分
- 备案链接 `truncate max-w-[140px]`（避免长 URL 撑爆布局）
- ⓘ 按钮 `aria-expanded` 跟随 state，键盘可达

### 8. 视觉验证
- Playwright 实测：折叠态 66px / 展开态 180px
- 有/无 LOGO 折叠态：66 / 66（差 0px）
- 折叠面板内：3 条外链（隐私/权限/备案） + 4 条产品功能 chip

### 9. v6 设计禁忌
- ❌ 不要把 ⓘ 详情按钮做成纯文字 "查看详情"（破坏 banner 极简感）
- ❌ 不要让折叠面板全屏（破坏"贴边"语义，最多 200px）
- ❌ 不要给 LOGO 设 min-height（容器固定 44x44 即可自适应）
- ❌ 不要在主条上塞产品功能（必须收进折叠面板，否则高度立刻膨胀）
